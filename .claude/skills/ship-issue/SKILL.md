...
- **Related Issue** — link to `https://github.com/holiday-calendar/holiday-calendar-ts/issues/<issue-number>`.
- **Type of Change** — check the box(es) that actually apply.
- **Checklist** — check off what's genuinely true (`pnpm test`/`pnpm typecheck`
  run locally, docs updated if needed, etc.); don't check boxes you haven't
  verified.
